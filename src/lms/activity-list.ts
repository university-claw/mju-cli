...
function parseMenuFromOnclick(
  ...
  const matched = onclickValue?.match(/,\s*'([^']+)'\s*,\s*'(\d+)'\s*,/);
  const menuId = matched?.[1];
  const activityId = parsePositiveInt(matched?.[2]);
...
function parseMenuFromHref(
  ...
  try {
    const url = new URL(href, "https://lms.mju.ac.kr");
    const menuId = url.searchParams.get("m")?.trim();
    const activityId = parsePositiveInt(url.searchParams.get("n") ?? undefined);
    if (!menuId || activityId === undefined) {
      return undefined;
    }
...
export function parseActivityListItems(html: string): ParsedActivityItem[] {
  ...
  $(".week_list_container").each((_, container) => {
    const section = $(container);
    const weekTitleWrap = section.find(".week_title_wrap").first();
    const week = parsePositiveInt(weekTitleWrap.attr("data-week"));
    const weekLabel =
      normalizeText(weekTitleWrap.find(".week_title > span").first().text()) || undefined;

    section.find("a.week_activity").each((_, element) => {
      const item = $(element);
      const menuInfo =
        parseMenuFromHref(item.attr("href")) ??
        parseMenuFromId(item.attr("id")) ??
        parseMenuFromOnclick(item.attr("onclick"));
      const title = normalizeText(item.find(".activity_title").first().text());
      if (!menuInfo || !title) {
        return;
      }

      const firstInfo = item.find(".activity_info").first();
      const statusLabel =
        normalizeText(firstInfo.find(".activity_info_title").first().text()) || undefined;
      const statusText =
        normalizeText(firstInfo.find(".activity_info_text").first().text()) || undefined;
      const attachmentCount = parsePositiveInt(
        normalizeText(item.find(".activity_attach_container .file").first().text())
      );
