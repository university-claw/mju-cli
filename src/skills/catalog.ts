...
export interface SkillCatalogEntry {
  ...
  kind: SkillKind;
  description: string;
  service?: "lms" | "msi" | "ucheck" | "library" | "safety";
  requires?: string[];
  relativePath: string;
  absolutePath: string;
}
...
const SKILL_SEEDS: SkillCatalogSeed[] = [
  {
    ...
    kind: "shared",
    description:
      "공통 인증, 출력, 안전 규칙, 캠퍼스/서비스 표면을 묶는 기본 skill",
  },
  {
    name: "mju-lms",
    kind: "service",
    service: "lms",
    description:
      "강의, 공지, 자료, 과제, 온라인 학습 흐름을 다루는 LMS 기본 skill",
    requires: ["mju-shared"],
  },
  {
    name: "mju-msi",
  ...
  },
  {
    ...
    service: "lms",
    description:
      "미제출 과제, 안읽은 공지, 미수강 온라인을 우선순위로 정리하는 helper skill",
  ...
  },
  {
    ...
    service: "library",
    description:
      "열람실 좌석 예약 preview, 생성, 취소 흐름을 안전하게 수행하는 helper skill",
    requires: ["mju-shared", "mju-library"],
  },
  {
  ...
  },
  {
    ...
    kind: "recipe",
    description:
      "오늘 처리할 LMS 액션과 도서관 예약 상황을 함께 확인하는 daily recipe",
  ...
  },
  {
    ...
    kind: "recipe",
    description:
      "LMS digest와 도서관 좌석 흐름을 묶어 학습 준비를 한 번에 점검하는 recipe",
